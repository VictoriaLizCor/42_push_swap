...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sorting_algorithm_copy.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lilizarr <lilizarr@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/19 11:17:58 by lilizarr          #+#    #+#             */
/*   Updated: 2023/03/09 10:32:23 by lilizarr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	middle_sort(t_stack **stack_a, t_stack **stack_b, int size)
{
	...
	// 	if ((*stack_b)->s_idx < (*stack_b)->next->s_idx && target->index == 2 && (*stack_b)->s_idx >= find_middle(*stack_b))
	// 		sb(*stack_b);
	// 	else if ((target->index < (find_max_value(*stack_b, &(*stack_b)->index) / 2) + 1 && target->index >= 2) || \
	// 	(*stack_b)->s_idx < find_middle(*stack_b))
	// 		rb((&*stack_b));
	// 	else if (target->index >= (find_max_value(*stack_b, &(*stack_b)->index) / 2) + 1 && target->index > 2)
	// 		rrb((&*stack_b));
...
void divide_stack(t_stack **stack_a, t_stack **stack_b, int size, int limit)
{
	...
	if ((*stack_a)->s_idx >= limit)
	{
		...
		if ((*stack_a)->s_idx - 1 == (*stack_a)->next->s_idx && \
		(*stack_a)->s_idx >= limit + 1 && (*stack_a)->next->s_idx >= limit)
		{
			sa(*stack_a);
		}
		else if (target->index >= size / 2 + 1)
			rra(&*stack_a);
		else
			ra(&*stack_a);
		print_stack(*stack_a);
...
void	divide_sort(t_stack **stack_a, t_stack **stack_b, int size)
{
	...
	// 	target1 = target2;
	ft_printf("taa -> %p \t\t\t|| \t index_tar =  %d || \t s_idx_tar =  %d\n", target1, target1->index, target1->s_idx);
	if (target1->index >= 2 && last_ordered_element(*stack_a) != *stack_a &&\
	!check_sorted(*stack_a) )
	{
		if (target1->index >= size / 2 + 1)
		{
			while (target1 != (*stack_a)->previous)
				rra(&*stack_a);
		}
		else if (target1->index < size / 2 + 1)
		{
			while (target1 != (*stack_a)->previous)
				ra(&*stack_a);
		}
...
void	sorting_algorithm(t_stack **stack_a, t_stack **stack_b)
{
	...
	// while (i++ < 3)
	{
		...
		if((*stack_a)->s_idx < find_max_s_idx(*stack_a) -  2 && !check_sorted(*stack_a))
			pb(&*stack_a, &*stack_b);
		while(*stack_b != NULL && (*stack_b)->s_idx == (*stack_a)->s_idx - 1 && \
		check_sorted(*stack_a))
		{
			pa(&*stack_b, &*stack_a);
		}
